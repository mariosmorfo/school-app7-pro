...
//            logging
    ...
    public Teacher update(Teacher teacher) throws TeacherDAOException {
        String sql = "UPDATE teachers SET firstname = ?, lastname = ?, vat = ?, fathername = ?, phone_num = ?, email = ?, " +
                "street = ?, street_num = ?, zipcode = ?, city_id = ?,  updated_at = ? " +
                "WHERE id = ?";
...
//            e.printStackTrace();
//            logging
            throw new TeacherDAOException("SQL Error, Insert with vat: " + teacher.getVat() + "not updated");
        }
    }
